...
pub struct CliArgs {
    ...
    #[arg(long, value_enum, default_value_t = CliFilterType::Lanczos3)]
...
fn main() -> Result<()> {
    ...
    // println!("{:#?}", args);
    args.block_height = Some(args.block_height.unwrap_or(args.block_width));
...
impl From<&CliArgs> for Operation {
    fn from(args: &CliArgs) -> Self {
        let from = if let Some(input_ext) = args.input.extension() {
        ...
        let to = if let Some(output_ext) = args.output.extension() {
...
fn run(Operation { from, to }: Operation, args: CliArgs) -> Result<()> {
    // println!("{:?} -> {:?}", from, to);
    match from {
        Type::Image => match to {
            Type::Pix => image_to_pix(args),
            Type::Image => image_to_image(args),
        },
        Type::Pix => match to {
            Type::Image => pix_to_image(&args),
            Type::Pix => pix_to_pix(args),
        },
    }
}
...
fn image_to_pix(
    ...
    pix.shrink_by(filter.into(), shrink_by);
    pix.save(&output)
...
fn image_to_image(
    ...
    let img = pix.to_image(filter).context(PIX_CVRT_ERROR)?;
    img.save(&output)
        .with_context(|| format_file_error(IMG_SAVE_ERROR, &output))?;
    Ok(())
}
#[inline]
fn pix_to_image(args: &CliArgs) -> Result<()> {
