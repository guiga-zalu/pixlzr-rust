...
fn encode_pix(pix: &Pixlzr) -> () {
    pix.encode_to_vec_vec().unwrap();
    ()
}
fn decode_png(data: &V) -> () {
    image::load_from_memory_with_format(data.as_slice(), image::ImageFormat::Png).unwrap();
    ()
}
...
pub fn criterion_benchmark(c: &mut Criterion) {
    ...
    let img = image::load_from_memory_with_format(&data_png, image::ImageFormat::Png).unwrap();
